...
export class ChessBoard {
  ...
  private isPositionSafeAfterMove(
    ...
    // We can't put piece on a square that already contains piece of the same square
      ...
      for (let y = 0; y < this.chessBoardSize; y++) {
        ...
        for (const { x: dx, y: dy } of piece.directions) {
          ...
          if (piece instanceof Pawn) {
            // cant move pawn two squares straight if there's a piece infront of it
            if (dx === 2 || dx === -2) {
              if (newPiece) continue;
              if (this.chessBoard[newX + (dx === 2 ? -1 : 1)][newY]) continue;
            }

            // cant move pawn one square straight if there's a piece infront of it
            if ((dx === 1 || dx === -1) && dy === 0 && newPiece) continue;

            // can't move pawn diagonally if there's no piece, or piece has same color as pawn
            if (
              (dy === 1 || dy === -1) &&
